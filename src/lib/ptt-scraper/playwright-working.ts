...
export class PTTWorkingScraper {
  private browser: Browser | null = null;
  private page: Page | null = null;
  private baseUrl = 'https://postakodu.ptt.gov.tr';

  async init() {
    console.log('🚀 PTT Working Scraper başlatılıyor...');
    
    this.browser = await chromium.launch({
      headless: true, // Hızlı çalışması için
  ...
  async scrapeAllData(): Promise<ScrapedData> {
    ...
    try {
      ...
      // Sadece ilk 3 ili test et
      const testIller = iller.slice(0, 3);
  ...
  async saveData(data: ScrapedData, filename?: string): Promise<string> {
    ...
    const defaultFilename = `ptt-addresses-working-${timestamp}.json`;
...
// Manuel çalıştırma
export async function runWorkingScraping() {
  const scraper = new PTTWorkingScraper();
  ...
  try {
    ...
    console.log('🎉 Working Scraping tamamlandı!');
    console.log(`📁 Dosya: ${filepath}`);
    console.log(`📊 Toplam: ${data.totalCount} adres`);
    
    return data;
  } catch (error) {
    console.error('❌ Working Scraping hatası:', error);
    throw error;
  } finally {
    await scraper.close();
  }
...
if (require.main === module) {
  runWorkingScraping().catch(console.error);
}
